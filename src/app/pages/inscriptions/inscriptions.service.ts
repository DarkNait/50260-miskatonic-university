...
import { catchError, concatMap, delay, finalize, map, of, tap, throwError } from "rxjs";
...
export class InscriptionsService {
    ...
    getInscriptionsByUserId(id: string | number) {
      return this.http
        .get<Inscription[]>(`${environment.apiURL}/inscriptions?_embed=user&_embed=course`)
        .pipe(   
          map((inscriptions) => {
            return inscriptions.filter((elem) => (elem.userId === id) );
          }),       
          catchError((error) => {
            this.alertService.showError('Ocurrio un error al recuperar las inscripciones del usuario.');
            return throwError(() => error);
          })
        );
    }
